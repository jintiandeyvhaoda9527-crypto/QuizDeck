...
- **企业部署选择：** 可配置兼容的云端接口，也可通过 HTTPS 内网网关接入 Ollama、vLLM 等 OpenAI 兼容服务。
...
通过本地或内网模型部署，企业可以让题库内容和学习数据保留在自身网络边界内。Android 与非本机网页连接要求使用 HTTPS，明文 HTTP 只用于本机回环地址开发。实际安全性仍取决于网络设计、访问权限、接口配置、设备策略和模型部署方式。
